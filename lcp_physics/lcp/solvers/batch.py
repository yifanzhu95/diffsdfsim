...
def solve_kkt_ir(Q, D, G, A, F, rx, rs, rz, ry, niter=1):
    ...
    res = kkt_resid_reg(Q, D, G, A, F_tilde, eps,
                        dx, ds, dz, dy, rx, rs, rz, ry)
    resx, ress, resz, resy = res
    res = resx
    for k in range(niter):
        ddx, dds, ddz, ddy = factor_solve_kkt_reg(Q_tilde, D_tilde, G, A, C_tilde, -resx, -ress, -resz,
...
""")
    ...
    try:
        ...
        # FIXME: Temporary bugfix for https://github.com/pytorch/pytorch/issues/36921
        if G.shape[0] == 1:
            G_invQ_GT = torch.bmm(G, G.squeeze(0).t().lu_solve(*[lu.squeeze(0) for lu in Q_LU]).unsqueeze(0)) + F
        else:
            raise err
